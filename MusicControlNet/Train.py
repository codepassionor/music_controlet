...
def train(args):
    ...
    prefix_token_params = filter(lambda p: p.requires_grad, loaded_prompt_learner.parameters())
    
    all_params = itertools.chain(unet_params, controlnet_params, prefix_token_params)
...
def main():
    ...
    parser.add_argument("--conditioning_data_dir", type=str, required=True, help="Path to conditioning data directory.")
    parser.add_argument("--output_dir", type=str, required=True, help="Directory to save the output.")
    parser.add_argument("--unet_model_name_or_path", type=str, required=True, help="Path to the pre-trained UNet model.")
    parser.add_argument("--num_genres", type=int, required=True, help="Number of genres in the dataset.")
    parser.add_argument("--num_moods", type=int, required=True, help="Number of moods in the dataset.")
    parser.add_argument("--num_controls", type=int, required=True, help="Number of control variables.")
    ...
    parser.add_argument("--min_snr_gamma", type=float, default=5.0, help="Minimum SNR gamma value.")
    parser.add_argument("--v_parameterization", action="store_true", help="Use v-parameterization if set.")
